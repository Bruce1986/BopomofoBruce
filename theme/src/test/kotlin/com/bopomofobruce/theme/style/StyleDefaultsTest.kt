 ...
 *
 * **它防得住什麼、防不住什麼（別高估）**：突變實證（2026-09-08）——單改預設值 → 本測試轉紅； 但**預設值與本測試的期望值一起改**（兩檔同動）→ 55
 * 條全綠。凡是簽入快照都有這個固有邊界。 它把「改預設值」從**零觸點**變成**一個觸點**：PR diff 裡會出現「production 預設值改了、測試 期望值也跟著改了」這組成對變更，讓
 * reviewer 有機會問「為什麼」。**擋下協同修改靠的是人看 diff，不是這條測試。**
 */
