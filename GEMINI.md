...
- **NDK**：`libchewing` 以子模組方式接入（`decoder-native/cmake/`），ABI 限定 arm64-v8a + armeabi-v7a，不打 x86。子模組要 `--recursive`（`data/` 是巢狀 submodule），否則 CMake 直接 `FATAL_ERROR`。Android SDK 位置由 `local.properties` 的 `sdk.dir`（或 `ANDROID_HOME`）指定；SDK 不在預設位置時（例如 Homebrew 的 `android-commandlinetools` 裝在 `/opt/homebrew/share/android-commandlinetools`），沒指對會被誤判成沒裝 SDK。
