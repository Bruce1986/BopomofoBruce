...
**`ViewTreeViewModelStoreOwner` 記憶體洩漏注意事項**：IME view 在服務生命週期內會多次銷毀重建（切輸入法、螢幕旋轉、focus 切換等）。若 `ViewModelStore` 綁到 `InputMethodService` lifecycle 而 ViewModel 內持有舊 view / Context reference，會 leak。W2-B (`:ime`) 實作 `BpmfInputMethodService` 規範：
