...
**`ViewTreeViewModelStoreOwner` 記憶體洩漏注意事項**：IME view 在服務生命週期內會多次銷毀重建（切輸入法、螢幕旋轉、focus 切換等）。若 `ViewModelStore` 綁到 `InputMethodService` lifecycle 而 ViewModel 內持有舊 view / Context reference，會 leak。W2-B (`:ime`) 實作 `BpmfInputMethodService` 時必須在 `InputMethodService.onDestroy()` 或自訂 `AbstractComposeView` 的 `onDetachedFromWindow()` 清空對應 `ViewModelStore`（或讓 ViewModel 完全不持 view / context 強引用） — 注意 `InputMethodService` **沒有** `onDestroyInputView` 這個 callback（不同於 Fragment）；W3-1 wiring reviewer 必檢查此點。
