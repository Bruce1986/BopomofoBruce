...
- 實測 `assembleDebug`／`assembleRelease` 都成功：debug `libbpmf.so` 未最終最佳化（arm64-v8a 11.4MB／armeabi-v7a 7.7MB，AGP 已剝除偵錯符號後的數字），release profile（workspace `Cargo.toml` 的 `[profile.release]` 已開 `lto = true, opt-level = 3, panic = "abort"`）大幅縮小：arm64-v8a 3.43MB／armeabi-v7a 2.45MB（剝除後）。（⚠️ 這是第十五階段 R1 之前的數字；R1 加上 `--version-script`／`--exclude-libs,ALL`／`--gc-sections` 後，release `.so` 為 arm64-v8a 1,592,400 bytes／armeabi-v7a 1,106,552 bytes，見 devlog 第十五階段。下方 K7 的 5.26 MiB 仍是以舊數字推算。）
...
- **（2026-08-11 第五階段新增）`:lint` 需要連網**：`package*Assets`/`Lint*` 兩類 task 現在都直接依附 `fetchChewingData`，這是修正下游 `:app` 拿不到字典（見 devlog E1）的必然代價——AGP 把 lint 的 model builder 無條件連到 `package*Assets`，這份 build script 無法切斷這條耦合，只能整批接受或整批拒絕。已刻意選擇「整批接受」：APK 正確性優先於 lint 可離線執行。離線環境跑 `:decoder-native:lint` 會失敗；CI 或斷網環境需注意這點。（2026-09-22 補記：`Lint*` 這批裡的自訂 lint 規則編譯／發佈任務——`compileLint`、`compileLintChecks`、`prepareLintJarForPublish`、`bundle*LocalLintAar`——不讀 assets，已從直接依附中排除；其中 `bundle*LocalLintAar` 仍可能經 `package*Assets` 間接需要連網。`lintAnalyze*` 等真正讀 assets 的任務維持直接依附，上面「整批接受」的結論不變。）
- **（2026-09-16 補記）native 建置本身需要連網，不只字典下載**：`decoder-native/cmake/CMakeLists.txt` 以 `FetchContent_Declare(Corrosion GIT_REPOSITORY https://github.com/corrosion-rs/corrosion.git ...)` 讓乾淨的建置目錄在 CMake configure 時 clone Corrosion（已以 commit 釘版）；`chewing_capi` 的 crates.io 依賴也沒有 vendor（`decoder-native/cmake/libchewing/.cargo/config.toml` 只有 `[alias]`、沒有 source replacement），本機沒有 cargo 快取時 `cargo build` 需要從 crates.io 取得。上面「`:lint` 需要連網」一條只涵蓋字典資料。若分發管道的建置環境不允許建置期連網（例如計畫中的 F-Droid，其建置政策本 repo 尚未查證），需要先 vendor Corrosion 與 crates 依賴；列為 W4-D 前待確認事項，本輪不動手。
- **（2026-09-16 補記）16 KB page 對齊要明確開啟**：釘版的 NDK r27 預設不開 `ANDROID_SUPPORT_FLEXIBLE_PAGE_SIZES`，實測先前建出的 arm64 `libbpmf.so` 三個 PT_LOAD 都只有 `0x1000` 對齊；`decoder-native/build.gradle.kts` 已加上 `-DANDROID_SUPPORT_FLEXIBLE_PAGE_SIZES=ON`（r27 的 `build/cmake/flags.cmake` 據此對 arm64-v8a 加 `-Wl,-z,max-page-size=16384`），重建後為 `0x4000`。CI 的「Verify packaged libbpmf.so ABIs」一併斷言打包進 APK 的 arm64 `libbpmf.so` 每個 PT_LOAD 對齊都 ≥ `0x4000`（只檢查我方的 `libbpmf.so`，不含其他相依套件的 `.so`）。
- **（2026-09-16 補記）Windows 原生環境跑不了字典下載**：`decoder-native/build.gradle.kts` 的 `fetchChewingData` 以 `commandLine("scripts/fetch_chewing_data.sh")` 直接執行 bash 腳本（靠 shebang 與執行位元），而 assets／lint／assemble 等 task 都依附它。Windows 原生環境不解析 shebang，需要 WSL 或 Git Bash；本 repo 目前只在 macOS 與 Linux CI 上建置過，Windows 未實測。
- **（2026-09-16 補記）CI 沒有自動跑 native 解碼器的實際行為**：`.github/workflows/ci.yml` 跑 `assembleDebug`、ELF ABI 檢查、ktfmt、lint 與 JVM 單元測試，沒有 emulator／`connectedAndroidTest`。`BpmfNativeSmokeTest`（`bpmf_init`／`bpmf_input` 真的在 `.so` 上跑）目前只在實機手動跑過（見 devlog），之後改 `bpmf_wrapper.c` 或升級 Rust／Corrosion 時 CI 全綠不代表解碼行為沒壞。要補 emulator job 還是明確接受這個缺口，留給 owner 決定。
